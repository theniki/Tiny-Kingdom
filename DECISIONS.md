...
## D-013 — Selection-aware action/collect sfx
- **Decision**: `villager-action`, `guard-action`, `collect-wood`, `collect-food`, and mining-path `collect-gold` only fire when the triggering unit is in the current selection. Training (`level-up`), upgrade/castle completion (`upgrade`), and treasure-chest `collect-gold` remain global (milestone events).
- **Alternatives considered**: (a) pure global sfx (current pain — chaotic overlap with 5+ villagers); (b) distance-based falloff (complex, distance-from-camera doesn't map cleanly to "what you're watching"); (c) silence everything (lifeless); (d) per-team cap only (doesn't reduce chaos meaningfully).
- **Why**: Selection is the most reliable proxy for "what the player is paying attention to." A selected worker's rhythmic chops are satisfying; an unselected village going about its business should feel peaceful. Milestones (training, upgrade, chest) bypass the rule because they're rare and narratively important — the player should hear those regardless of camera or selection focus.
- **Tradeoffs accepted**: Selecting 5 villagers still allows up to 3 simultaneous `villager-action` sfx (existing cap). A player who never selects anyone hears no action sfx — acceptable given the loop is visually self-evident. Debug `window.audioDebug.forceAllSfx = true` restores pre-D-013 behavior for A/B testing.

## D-012 — Dynamic music states (peaceful vs combat)
- **Decision**: Music follows two states. **Peaceful** randomly alternates between `theme-normal.mp3` and `ambient-layer.mp3` (50/50, new random pick at each natural end, no loop). **Combat** plays `theme-attack.mp3` on loop. Combat is detected by any soldier in the `attacking` state with a live target OR any entity taking HP damage, with a 5-second cooldown before combat→peaceful. Transitions crossfade: 1500ms into combat (urgent), 3000ms back to peaceful (relieving).
- **Alternatives considered**: (a) single looping peaceful track; (b) the fixed theme×2→ambient cycle from D-011; (c) fully procedural music; (d) music that reacts to specific events (one track per event type).
- **Why**: Reactive music tells the player the stakes have changed — a core AAA-game feel at minimal engineering cost. Two peaceful tracks prevents fatigue over a 20-min session without requiring a large library. The 5-second cooldown is the critical polish: without it, small skirmishes flip the music state repeatedly within seconds and sound broken. Single-loop (a) was boring in testing (kid called it "stuck"). Fixed cycle (b) from D-011 ignored gameplay entirely. Fully procedural (c) is overkill. Event-triggered music (d) is chaotic for young players.
- **Tradeoffs accepted**: Combat detection is heuristic — a distant 1-on-1 skirmish on the far side of the map still triggers combat music even if the player's base is peaceful. Acceptable because combat anywhere is narratively tense. HP-delta detection checks every 250ms, so the fastest combat music can trigger is ~250ms after first hit — imperceptible. Supersedes D-011 (fixed cycle removed from the codebase).

## D-011 — Music cycle: theme ×2 → ambient (60s) → repeat, with crossfades
- **Decision**: In-game music follows a fixed cycle — `background-theme-normal.mp3` plays twice in a row, then a 2-second crossfade to `ambient-layer.mp3` looping for 60 seconds, then a 2-second crossfade back to the theme. The cycle repeats for the life of the session.
- **Alternatives considered**: (a) single looping theme; (b) random shuffle across the two variants; (c) dynamic combat-reactive music switching to `background-theme-attack.mp3` during waves.
- **Why**: A kid will hear the theme many times in a 15–20 min session. A single loop becomes fatiguing within the first loop. The ×2 + ambient pattern gives the theme breathing room while keeping a familiar identity — the melody returns often enough to anchor the mood but rests long enough to stay fresh. Crossfades give AAA-quality transitions with ~20 lines of tween code. Dynamic-combat music was ruled out as scope creep for a kid-friendly game — the wave warning toast + visual escalation already signal tension without a soundtrack change.
- **Tradeoffs accepted**: Fixed schedule ignores game state (ambient plays even during combat). The `background-theme-attack.mp3` track is currently unused — BACKLOG entry for future dynamic music. Crossfade window (2s) means both tracks overlap briefly — acceptable because the ambient layer is low-intensity by design.
